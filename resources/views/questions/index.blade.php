<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto mt-10 px-4">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ $category === 'spesifik' ? 'Pertanyaan Khusus Jurusan' : 'Pertanyaan Umum' }}
        </h2>

        <form action="{{ $category === 'spesifik' ? route('recommendations.finalResult') : route('questions.submitAll') }}"
                                ...
                                @if ($question->answer_type === 'boolean')
                                    ...
                                    <div class="flex flex-col sm:flex-row gap-4">
                                        <label for="yes_{{ $question->id }}" class="inline-flex items-center">
                                            <input id="yes_{{ $question->id }}" type="radio" name="{{ $name }}" value="1" class="form-radio text-green-600" required>
            ...
            @if(!$questions->isEmpty())
                <div class="text-center">
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-6 py-3 rounded-lg shadow transition-all">
                        Simpan Jawaban
                    </button>
                </div>
            @endif
